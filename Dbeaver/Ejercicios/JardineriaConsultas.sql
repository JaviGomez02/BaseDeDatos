...
AND p.FECHA_PEDIDO>p.FECHA_ESPERADA;
...
SELECT E.CODIGO_EMPLEADO ,COUNT(C.NOMBRE_CLIENTE)
FROM EMPLEADO E , CLIENTE C
WHERE E.CODIGO_EMPLEADO=C.CODIGO_EMPLEADO_REP_VENTAS
...
--SUBCONSULTAS: DE AQUI PARA ABAJO ESTAN TODAS BIEN
...
FROM EMPLEADO e
WHERE e.CODIGO_EMPLEADO NOT IN (SELECT E2.CODIGO_EMPLEADO FROM CLIENTE c, EMPLEADO e2 
...
FROM PRODUCTO p, DETALLE_PEDIDO dp 
WHERE p.CODIGO_PRODUCTO=dp.CODIGO_PRODUCTO
AND dp.CANTIDAD=(SELECT MAX(CANTIDAD) FROM DETALLE_PEDIDO);
...
SELECT  DISTINCT C.NOMBRE_CLIENTE,NVL(COUNT(P.CODIGO_PEDIDO),0) 
FROM CLIENTE C ,PEDIDO P
WHERE C.CODIGO_CLIENTE=P.CODIGO_CLIENTE(+)
...
SELECT DISTINCT C.NOMBRE_CLIENTE,NVL(SUM(P.TOTAL),0)
FROM CLIENTE C, PAGO P
WHERE C.CODIGO_CLIENTE=P.CODIGO_CLIENTE(+)
GROUP BY C.NOMBRE_CLIENTE;
